  ...
  const PANELS = [
    {
      ...
      html: `
        ...
        <ol class="help-steps">
          <li><strong>定调</strong>：旋律轨名称旁点「调」「阶」为<strong>该轨</strong>设根音与音阶；换乐器会恢复该乐器的常用默认（如小号 B♭ 大调、小提琴 G 大调）。鼓轨无音高，不设调/阶。</li>
    ...
    },
    {
      ...
      html: `
        ...
        <ul class="help-list">
          ...
          <li><strong>调 / 阶</strong>：旋律轨旁的音名与音阶名（如 C、大调）；仅影响该轨选音。</li>
    ...
    },
    {
      ...
      html: `
        <h4>程序音高基础</h4>
        <p>旋律轨每格存一个 <strong>MIDI 音高</strong>（数字），界面写成 <code>音名+八度</code>（如 <code>G2</code>、<code>C4</code>）。每条旋律轨有<strong>自己的调与阶</strong>（轨名旁的按钮）；选音弹窗内「本格覆盖」可单独改某一格的音阶，点「跟轨」恢复该轨默认。音阶可选：大调、小调、五声、蓝调（已去掉不常用的多利亚）。</p>
        ...
        <h4>不同调式与分轨示例</h4>
        <p>换「阶」为五声时选音变少；蓝调适合萨克斯等；小调时贝斯 / 和弦优先 i、iv、V 级。小号、长号默认可为 B♭ 大调，小提琴可为 G 大调，与钢琴 C 大调可并存（分轨各管各的）。改调/阶后已填音不会自动移调，需手动重选。</p>
      `,
    },
    {
      id: "mixer",
