...
const BeatBattleCloud = (() => {
  ...
  async function publishWork({ title, audioBlob, userName }) {
    ...
    if (publishForm) {
      publishForm.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          AppLogger.info("已发布到制作库", `${work.title} · ${work.id.slice(0, 8)}`);
          setStatus?.(`已发布「${work.title}」— 请到评阅站制作库提交参赛`);
          syncHeaderBadge();
        } catch (err) {
